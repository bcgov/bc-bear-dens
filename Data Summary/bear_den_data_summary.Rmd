...
.footnote[*Still thinking on the best way to categorize/utilize "Active Recently (0-4 seasons).*]
...
f <- f |> 
  dplyr::group_by(den_id) |>
  dplyr::mutate(consecutiveuse_yn = ifelse(cumulative_visit == 1,
...
chisq <- f |>
  dplyr::mutate(den_status_binary = dplyr::case_when(grepl("No", den_status) ~ "Not Active",
                                             ...
                                             TRUE ~ "Unknown")) |>
  dplyr::filter(den_status_binary %in% c("Active", "Not Active")) |>
  dplyr::group_by(hair_on_entrance, den_status_binary) |>
  dplyr::tally() |>
...
Let's take a look at what the first modeling pass looked like.
...
den status ~ % old growth        # do dons with higher % old growth nearby have more activity?
den status ~ % new growth        # do dens with higher % new growth nearby have less activity?
```


A priori sets:
- Den status versus stand level forestry metrics (% forested within 60m, distance to edge of block, etc.)
- Den status versus landscape level metrics (road density within 1.5 km, % old growth within 1.5 km, etc.)
- Den status versus stand-landscape interaction (e.g., would a high quality stand within a low quality landscape attract more bears?)
- Den status versus den characterstics (is a nicer den more used, regardless of the forest around it?)
  - Can't directly compare to forestry models bc different sample sizes
...
TODO: use the "has a den ever been used" as the response for the boxplots

---
class: center, middle, inverse

# Question 3: Does windthrow affect den activity?
