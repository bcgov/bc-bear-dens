...
model0a <- glm(den_status_binary ~ v_prop_forest_60m + age + year + region, data = dat, family = binomial)
model0b <- glm(den_status_binary ~ dist_lt40 + year + region, data = dat, family = binomial) # can't have age in this!!!
model0c <- glm(den_status_binary ~ dist_gt40 + year + region, data = dat, family = binomial) # can't have age in this!!
model0d <- glm(den_status_binary ~ age + year + region, data = dat, family = binomial)
model0e <- glm(den_status_binary ~ dist_from_edge + age + year + region, data = dat, family = binomial)
model0f1 <- glm(den_status_binary ~ dist_from_edge * age + year + region, data = dat, family = binomial)
...
model0g1 <- glm(den_status_binary ~ dist_road + age + year + region, data = dat, family = binomial)
model0g2 <- glm(den_status_binary ~ dist_road * age + year + region, data = dat, family = binomial)
...
model0i <- glm(den_status_binary ~ gt_8 + age + year + region, data = dat, family = binomial)
model0j <- glm(den_status_binary ~ lt_3 + age + year + region, data = dat, family = binomial)
model0k <- glm(den_status_binary ~ road_density_m2 + age + year + region, data = dat, family = binomial)
model0l <- glm(den_status_binary ~ gt_8 + dist_lt40 + age + year + region, data = dat, family = binomial)
model0m <- glm(den_status_binary ~ gt_8 + v_prop_forest_60m + age + year + region, data = dat, family = binomial)
model0n <- glm(den_status_binary ~ road_density_m2 + dist_road * age + year + region, data = dat, family = binomial)
...
bbmle::AICtab(models_list, weights = TRUE, base = TRUE)
```
...
Same model, but with continuous predictors rescaled and centered on their means to make coefficient comparison easier to interpret. 

```{r top model summary rescaled}
jtools::summ(get(top_model), scale = TRUE, center = TRUE, confint = TRUE)
```
...
```{r den status v dist_road x age pred for publication, fig.height = 5, include = FALSE}
sjPlot::plot_model(get(top_model), type = "pred", terms = c("dist_road[all]", "age")) + 
  scale_color_manual(values = c("cadetblue", "coral"), labels = c("Immature", "Mature")) +
  scale_fill_manual(values = c("cadetblue", "coral"), labels = c("Immature", "Mature")) +
  labs(x = "Distance to Nearest Road (m)",
       y = "Predicted den activity probability",
       col = "AGE") +
  theme_minimal() +
