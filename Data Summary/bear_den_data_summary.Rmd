...
# Analysis options
den_status_strict <- FALSE # should the first record of "Active recently (0-4 seasons)" be counted as "Active"? TRUE or FALSE
...
if (den_status_strict == FALSE) {
  cat('.footnote[\\* If the first datapoint in the timeseries was `Active recently (0-4 seasons)`, we counted it as `Active` (*N* = `r nrow(f[f$den_status == "Active recently (0-4 seasons)" & f$den_status_binary == "Active", ])`).]')
}
```
...
model0a <- glm(den_status_binary ~ f_prop_forest_60m + age + year, data = dat, family = binomial)
model0b <- glm(den_status_binary ~ log_dist_lt40 + age + year, data = dat, family = binomial)
model0c <- glm(den_status_binary ~ log_dist_gt40 + age + year, data = dat, family = binomial)
model0d <- glm(den_status_binary ~ age + year, data = dat, family = binomial)
model0e <- glm(den_status_binary ~ log_dist_from_edge + age + year, data = dat, family = binomial)
model0f1 <- glm(den_status_binary ~ log_dist_from_edge * age + year, data = dat, family = binomial)
model0f2 <- glm(den_status_binary ~ log_dist_from_edge * age * year, data = dat, family = binomial)
model0f3 <- glm(den_status_binary ~ log_dist_from_edge * forestry_treatment_desc + year, data = dat, family = binomial)
model0g1 <- glm(den_status_binary ~ log_dist_road + age + year, data = dat, family = binomial)
model0g2 <- glm(den_status_binary ~ log_dist_road * age + year, data = dat, family = binomial)
model0g3 <- glm(den_status_binary ~ log_dist_road * age * year, data = dat, family = binomial)
model0h <- glm(den_status_binary ~ log_dist_road * forestry_treatment_desc + year, data = dat, family = binomial)
...
model0l <- glm(den_status_binary ~ gt_8 + log_dist_lt40 + age + year, data = dat, family = binomial)
model0m <- glm(den_status_binary ~ gt_8 + f_prop_forest_60m + age + year, data = dat, family = binomial)
model0n <- glm(den_status_binary ~ road_density_m2 + log_dist_road + age + year, data = dat, family = binomial)
model0o <- glm(den_status_binary ~ log_dist_lt40 * region + age + year, data = dat, family = binomial)

model1a <- glm(den_status_binary ~ year + region + f_prop_forest_60m + log_dist_from_edge * age + road_density_m2, data = dat, family = binomial)
model1b <- glm(den_status_binary ~ year + region + f_prop_forest_60m + log_dist_road * age, data = dat, family = binomial)
...
## Question 1: Top Model 1 Summary

The relationship is different between young stands versus old, mature stands. In general, dens within old/mature stands tend to be further from the cutblock edge, while dens within young/cutblock stands tend to be close to the edge of mature forests. 

```{r r den status v age x dist_from_edge boxplot, fig.height = 5}
dat |>
...
As distance from the cutblock edge increases, the chances of being "Active" also go up.

```{r effect plot dist_from_edge, fig.height = 5}
sjPlot::plot_model(get(top_model), type = "pred", terms = c("log_dist_from_edge"))
```
...
```{r den status v dist_from_edge x age effect plot, fig.height = 5}
sjPlot::plot_model(get(top_model), type = "pred", terms = c("log_dist_from_edge", "age"))
```
...
If you want to get even more fine grained, the relationship between "Active" probability and the _type_ of forest stand is variable, though not statistically significant.

```{r predictor plot dist_from_edge x patch type, fig.height = 5}
sjPlot::plot_model(glm(den_status_binary ~ log_dist_from_edge * forestry_treatment_desc, data = dat, family = binomial), type = "pred", terms = c("log_dist_from_edge", "forestry_treatment_desc"))
```
...
The next-best fit is the model that incorporates distance from forest road, rather than distance from cutblock edge.
...
This model tells a similar story! If a den is **close to a road** and **within a cutblock**, it's statistically more likely to be **NOT ACTIVE!**
...
In the case of the roads, it doesn't matter what the age of the stand is: the further away from the road you are, the more likely you are to find an Active den.

```{r effect plot dist_road x age, fig.height = 5}
sjPlot::plot_model(get(top_model2), type = "pred", terms = c("log_dist_road", "age"))
```
...
The full "kitchen sink" model is the best fit in this case. It shows that for dens that are in old forest and further from the cutblock edge, there is a higher probability of finding some bear hairs.
...
model0r <- glm(y ~ region + age + year, data = dat, family = binomial)
model0a <- glm(y ~ f_prop_forest_60m + age + year, data = dat, family = binomial)
model0b <- glm(y ~ log_dist_lt40 + age + year, data = dat, family = binomial)
model0c <- glm(y ~ log_dist_gt40 + age + year, data = dat, family = binomial)
model0d <- glm(y ~ age + year, data = dat, family = binomial)
model0e <- glm(y ~ log_dist_from_edge + age + year, data = dat, family = binomial)
model0f <- glm(y ~ log_dist_from_edge * age + year, data = dat, family = binomial)
model0g <- glm(y ~ log_dist_road + age + year, data = dat, family = binomial)
model0h <- glm(y ~ log_dist_road * age + year, data = dat, family = binomial)
model0i <- glm(y ~ gt_8 + age + year, data = dat, family = binomial)
model0j <- glm(y ~ lt_3 + age + year, data = dat, family = binomial)
model0k <- glm(y ~ road_density_m2 + age + year, data = dat, family = binomial)
model0l <- glm(y ~ gt_8 + log_dist_lt40 + age + year, data = dat, family = binomial)
model0m <- glm(y ~ gt_8 + f_prop_forest_60m + age + year, data = dat, family = binomial)
model0n <- glm(y ~ road_density_m2 + log_dist_road + age + year, data = dat, family = binomial)
model0o <- glm(y ~ log_dist_lt40 * region + age + year, data = dat, family = binomial)
...
- `year` explained much of the variance - likely because we visited many more dens in later years rather than older years.
- Distance from cutblock edge and distance from forest road jumped out as important. In general, dens are more likely to be active the further from a cutblock edge or forestry road that they are!
...
```{r den status v den measurements, dev.args=list(bg="transparent")}
label_names <- c(
